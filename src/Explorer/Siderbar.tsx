...
import { VortexData, FilteredVortexData } from "./DataContext";
import { setMinMax, compareMinMax } from "../ShapeUtils/MinMax";

export default function Sidebar({}) {
    const { vortex_data } = useContext(VortexData);
    const { setFilteredVortexData } = useContext(FilteredVortexData);
    ...
    const [perijove, setPerijove] = useState({
        minValue: 13,
        currentMin: 13,
        currentMax: 36,
        maxValue: 36,
    ...
    useEffect(() => {
        if (vortex_data.length > 0) {
            ...
            const aspect_ratios = vortex_data.map(
    ...
    return (
        <div className="py-2 px-4 col-span-1 bg-primary-200 min-h-screen box-border [&>*]:my-6">
            ...
            <InputMultiRange
                ...
                defaultMin={size.minValue}
                defaultMax={size.maxValue}
            />
            <InputMultiRange
                key={aspect_ratio.minValue + aspect_ratio.maxValue}
                minValue={aspect_ratio.minValue}
                maxValue={aspect_ratio.maxValue}
                step={0.5}
                type="float"
                text={"Aspect Ratio"}
                onChange={(minValue, maxValue) =>
                ...
                defaultMin={aspect_ratio.minValue}
                defaultMax={aspect_ratio.maxValue}
            />
            <InputMultiRange
                key={perijove.minValue + perijove.maxValue}
                minValue={perijove.minValue}
                maxValue={perijove.maxValue}
                type="int"
                step={1}
                text={"Perijove"}
                onChange={(minValue, maxValue) =>
                    setPerijove((prevState) => ({
                        ...prevState,
                        currentMin: minValue,
                        currentMax: maxValue,
                    }))
                }
                defaultMin={perijove.minValue}
                defaultMax={perijove.maxValue}
            />
            <InputMultiRange
                key={num_extract.minValue + num_extract.maxValue}
                minValue={num_extract.minValue}
                maxValue={num_extract.maxValue}
                step={1}
                type="int"
                text={"# of extracts"}
                onChange={(minValue, maxValue) =>
                    setNumExtracts((prevState) => ({
                        ...prevState,
                        currentMin: minValue,
                        currentMax: maxValue,
                    }))
                }
                defaultMin={num_extract.minValue}
                defaultMax={num_extract.maxValue}
            />
            <div className="w-full m-0 flex flex-col">
                <Checkbox
                    key={"white"}
                    value={colors_checked.white}
                    text="White vortices"
                    name="white_vortices"
                    onChange={(e) =>
                        setColorsChecked((prevState) => ({
                            ...prevState,
                ...
                <Checkbox
                    key={"brown"}
                    value={colors_checked.brown}
                    text="Brown vortices"
                    name="brown_vortices"
                    onChange={(e) =>
                        setColorsChecked((prevState) => ({
                            ...prevState,
                            brown: !prevState.brown,
                        }))
                    }
                />
                <Checkbox
                    key={"dark"}
                    value={colors_checked.dark}
                    text="Dark vortices"
                    name="dark_vortices"
                    onChange={(e) =>
                        setColorsChecked((prevState) => ({
